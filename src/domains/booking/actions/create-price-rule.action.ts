...
export async function createPriceRuleAction(
  ...
  try {
    ...
    const rule = await getPriceRuleService().createRuleForOwnerCourt(
      ownerId,
      parsed.data.courtId,
      parsed.data,
    );
    ...
    return handleServerActionError("createPriceRuleAction", error, {
      fallbackMessage: "Gagal membuat aturan harga.",
      knownErrors: [
